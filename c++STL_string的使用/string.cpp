...
//c_str  返回c形式的字符串（返回的是字符串数组的指针）：遇到'\0'就结束，不会再输出'\0'后面的字符

int main()
{
	string s("hello world");
	const char* str = s.c_str();  //c_str（）返回的是字符串指针，字符串末尾一定会有'\0'。
	while (str)
	{
		cout << *str << " ";
		++str;
	...
	cout << endl;
	cout << s << endl;//调用string重载的operator<<函数  :将对象数组中的所有字符都输出。不会因为遇到'\0'结束输出
	cout << s.c_str() << endl;//字节输出const char* 
	/*s += '\0';
	s += "world";
	cout << s << endl;
	cout << s.c_str() << endl;*/
